...
description: >
  ...
  display + Inter body + ui-monospace for the index/code voice on a warm-navy code surface,
  sentence-case display, and the ✱ coral spike mark. Composition + frame scale rewritten. Motion out
  of scope.
unit: the frame — 1920×1080 primary; 9:16 and 1:1 documented
principle: atoms are sacred · composition is free · numbers come from the script

# Inverted from the preset's light-paper default. The film cuts against
# screen recordings of a dark application; a cream ground would strobe at
# every seam. `ink` is text, `cream` is the ground — so they swap, and the
# app's own mint becomes the scarce accent in place of the preset's coral.
colors:
  ink: "#efece4"
  cream: "#16150f"
  tile: "#1b2f27"
...
borders: { hairline: "1px solid ink@14%", hairline-strong: "1px solid ink@24%", dark: "1px solid ink@14% (on navy)" }
shadows: { card: "none — depth comes from surface steps, not shadow, on a dark ground", none: "none" }
...
typography:
  ...
  kicker:  { fontFamily: "ui-monospace", px: 28, cqw: 1.46, weight: 500, tracking: "0.16em", upper: true }
  mono-label:{ fontFamily: "ui-monospace", px: 26, cqw: 1.35, weight: 500, tracking: "0.02em" }
  code:    { fontFamily: "ui-monospace", cqw: 1.67, weight: 400, lineHeight: 1.6 }
  ...
  number-unit:{ fontFamily: "ui-monospace", cqw: 2.08, weight: 500, lineHeight: 1.0 }
...
components:
  ...
  kicker-spike:
    ...
    description: "The eyebrow — ui-monospace uppercase, indexical (2–5 words), prefixed with the coral ✱. Never plain text, never a sentence."
  coral-callout:
  ...
  number-lockup:
    ...
    description: "Hero stat — a Inter figure paired with a ui-monospace unit (200K, +1,204, −318, 17 files). Figure is serif; the unit is ALWAYS mono, never the serif."
  pull-quote:
  ...
  code-surface:
    ...
    textColor: "{colors.cream} (ui-monospace); syntax in coral (keywords) / teal #5DB8A6 (strings) / amber #E8A55A (numbers)"
...
UI chrome. **ui-monospace** carries the indexical layer — kickers, technical labels, the code
...
- **Inter** (sentence case, negative-tracked) for all display; **Inter** body/chrome; **ui-monospace** index + code.
...
- **Type** — Inter sentence-case display (negative-tracked); Inter 400 body; ui-monospace kickers (uppercase 0.16em, coral ✱) + code.
...
- **Inter display is sentence case** (NOT title case, NOT uppercase), weight 400, negative-tracked (−0.012..−0.028em); reach for **italic** when the line is a stance or a definition. **Inter body** sentence case weight 400. **ui-monospace** kickers UPPERCASE 0.16em with the coral ✱. No uppercase serif, no sans headline, no serif label.
...
- Set all display in **Inter, sentence case**, negative-tracked; **Inter 400** body; **ui-monospace** kickers (uppercase, 0.16em, coral ✱) + code.
...
- **Type** — Inter sentence-case display (negative-tracked); Inter 400 body; ui-monospace kickers (uppercase 0.16em, coral ✱) + code; ≥1.4cqw floor.
